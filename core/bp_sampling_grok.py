...
if __name__ == "__main__":
    ...
    P = np.array(
        [
            [0, 0.5, 0, 0.5, 0, 0, 0, 0],  # P(X_{t+1} | X_t = 0)
            [0.5, 0, 0.5, 0, 0, 0, 0, 0],  # P(X_{t+1} | X_t = 0)
            [0, 0.5, 0, 0.5, 0, 0, 0, 0],  # P(X_{t+1} | X_t = 0)
            [0, 0, 0.5, 0, 0.5, 0, 0, 0],  # P(X_{t+1} | X_t = 0)
            [0, 0, 0, 0.5, 0, 0.5, 0, 0],  # P(X_{t+1} | X_t = 0)
            [0, 0, 0, 0, 0.5, 0, 0.5, 0],  # P(X_{t+1} | X_t = 0)
            [0, 0, 0, 0, 0, 0.5, 0, 0.5],  # P(X_{t+1} | X_t = 0)
            [0.5, 0, 0, 0, 0, 0, 0, 0.5],  # P(X_{t+1} | X_t = 0)
        ]
    )
    n = 5

    # # Test with constraints at various positions
    # constraints = {1: 0, 3: 1}  # X_1 = 0, X_3 = 1, others free
    # seq = sample_sequence_with_constraints(P, constraints, n)
    # print("Sequence with constraints {1: 0, 3: 1}:", seq)
    #
    # # Test with constraints including ends
    # constraints = {1: 0, 3: 1, 5: 2}
    # seq = sample_sequence_with_constraints(P, constraints, n)
    # print("Sequence with constraints {1: 0, 3: 1, 5: 2}:", seq)
    #
    # # Test with no constraints
    # constraints = {}
    # seq = sample_sequence_with_constraints(P, constraints, n)
    # print("Sequence with no constraints:", seq)
    #
    # Test with only one constraint
    constraints = {1: 5}
    seq = sample_sequence_with_constraints(P, constraints, n)
    print("Sequence with constraint {2: 1}:", seq)
