...
class MaxEnt:
    ...
    """
    The indices of the contexts in J, the 3D-array of interaction potentials used
    to compute the total interaction potential for each context; the shape of L_ix_arr
    is (M, 3, 2•kmax), the typical usage is Z_ix_arr[µ] to get the indices
    of the interaction potentials for the context centered around symbol µ: the result
    is a 3D-array of shape (3, 2•kmax) such that J[L_ix_arr[µ]] contains all the 
    interaction potentials for the context centered around symbol µ at all distances.
    """
    L_ix_arr: NDArrayInt

    # The indices of the contexts in J, the 3D-array of interaction potentials used
    # to compute the total interaction potential for each context when replacing the
    # center of the context with a symbol from the vocabulary; the shape of Z_ix_arr
    # is (M, 3, q, 2•kmax), the typical usage is Z_ix_arr[µ, σ] to get the indices of
    # the interaction potentials of the context centered around symbol µ with the center
    # replaced by symbol σ: the result is a 2D-array of shape (3, 2•kmax) such that
    # J[Z_ix_arr[µ, σ]] contains all the interaction potentials for the context centered
    # around symbol µ with the center replaced by symbol σ at all distances.
